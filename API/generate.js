...
module.exports = async (req, res) => {
    cors(req, res, async () => {
        ...
        try {
            ...
            // Validate input
            if (!cdk) {
                return res.status(400).json({ error: 'CDK key is required' });
            }
            if (!sessionToken) {
            ...
            // Check if CDK has usage limit
            const maxUses = cdkData.maxUses || 1;
            const currentUses = cdkData.uses || 0;

            if (currentUses >= maxUses) {
                return res.status(400).json({ error: 'CDK usage limit exceeded' });
            }
            ...
            // If maxUses reached, mark as used
            if (newUses >= maxUses) {
